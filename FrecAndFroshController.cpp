...
FrecAndFroshController::~FrecAndFroshController() {
	for (auto &i : allThrowFrecs) { //same as for i < allThrowFrecs.size()
		allThrowFrecs.erase(allThrowFrecs.begin(), allThrowFrecs.end());
	}
	//do for each special vector
}

/*______________________________________________THROW FREC_______________________________________________________*/

void FrecAndFroshController::addThrowFrecToList(sf::Vector2f position, int index) {
	allThrowFrecs.push_back(make_unique<Frec>(position, index));
}
void FrecAndFroshController::deleteThrowFrecAtIndex(int index) {
	allThrowFrecs.erase(allThrowFrecs.begin() + index, allThrowFrecs.begin() + index + 1);
	//delete image
}
void FrecAndFroshController::drawAllThrowFrecsOnGrid(sf::RenderWindow &theWindow) {//draws the frec onto the grid
	for (int i=0; i <allThrowFrecs.size(); i++) { //for each throw frec
		allThrowFrecs[i]->drawThrowFrec(theWindow);
	}
}
/*______________________________________________THROW FREC_______________________________________________________*/
...
void FrecAndFroshController::addThrowObjectToList(int index, sf::Vector2f frecPosition, shared_ptr<Frosh> froshPtr) {
	allThrowObjects.push_back(make_unique<throwProjectile>(index, frecPosition, froshPtr));
}
void FrecAndFroshController::deleteThrowObjectAtIndex(int index) {
	allThrowObjects[index].reset();
	allThrowObjects.erase(allThrowObjects.begin() + index, allThrowObjects.begin() + index + 1);
	//delete image? I dont think you'll need to explicitly do this
	//delete &allThrowObjects[index];
}
void FrecAndFroshController::drawAllThrowObjectsOnGrid(sf::RenderWindow &theWindow) {//draws the frec onto the grid
	for (int i = 0; i < allThrowObjects.size(); i++) { //for each throw frec
		allThrowObjects[i]->drawProjectile(theWindow);
	}
}
/*__________________________________________OBJECT BEING THROWN_________________________________________________*/


//_________Frosh Testing____________
void FrecAndFroshController::drawAllFrosh(sf::RenderWindow &theWindow) {//draws the frec onto the grid
	for (int i = 0; i < allFrosh.size(); i++) { //for each frosh
		allFrosh[i]->render(&theWindow);
	}
}
void FrecAndFroshController::addFroshObjectToList(sf::Vector2f _position, sf::Vector2f _size, sf::Texture* _texture,
	int type, double modifier) {
	allFrosh.push_back(make_unique<Frosh>(_position, _size, _texture, type, modifier));
}
//_________Frosh Testing____________
...
void FrecAndFroshController::moveAllThrowObjectTowardsFroshAndDelete() { //this parameter will be replaced by allFrosh[j]->getPosition() in each if statement
	for (int i = 0; i < allThrowObjects.size(); i++) { //for each throw object
		...
		if (allThrowObjects[i]->projectileFroshCollision(allThrowObjects[i]->getFroshTarget()->getCenterPosition())) { 
			//deal damage to frosh
			allThrowObjects[i]->getFroshTarget()->reduceHealth(allThrowObjects[i]->getFroshTarget()->getHealth());
			...
			allThrowObjects[i]->moveObjectTowardsFrosh(allThrowObjects[i]->getFroshTarget()->getCenterPosition());
		}
	}
}
...
void FrecAndFroshController::updateFrecFroshRange() {
	...
	if (allThrowFrecsInRangeOfFrosh.size() > 0)
		allThrowFrecsInRangeOfFrosh.erase(allThrowFrecsInRangeOfFrosh.begin(), allThrowFrecsInRangeOfFrosh.end());
	 //clear contents of 
	if (allFroshInRangeOfFrecs.size() > 0)
		allFroshInRangeOfFrecs.erase(allFroshInRangeOfFrecs.begin(), allFroshInRangeOfFrecs.end());
	
	for (int i = 0; i < allThrowFrecs.size(); i++) { //for each frec object
		for (int j = 0; j < allFrosh.size(); j++) { //for each frosh object
			if (allThrowFrecs[i]->CollidesWithFrec(allFrosh[j]->getPosition())) {
...
void FrecAndFroshController::process() { //essentially do every attack related function in a small time tick within process
	...
	for (int i = 0; i < allThrowFrecs.size(); i++) {
	...
	for (int i = 0; i < allThrowFrecsInRangeOfFrosh.size(); i++) { //frecs and frosh will have same index i
		if (allThrowFrecsInRangeOfFrosh[i]->getCooldown() == 0) { //if the frec is not on cooldown fire a new projectile, else do nothing
			...
			addThrowObjectToList(allThrowObjects.size() - 1, allThrowFrecsInRangeOfFrosh[i]->getFrecPosition(), allFroshInRangeOfFrecs[i]); //append a throw object to the end of the vector
...
void FrecAndFroshController::render(sf::RenderWindow &_window) {
	...
	drawAllThrowFrecsOnGrid(_window);
	drawAllFrosh(_window);
}
